...
public class UseItem {
	...
	public static void ItemonNpc(Client c, int itemId, int npcId, int slot) {
		switch(itemId) {
			//case for coin pouches however need to make own class extending this method
			//made by K3LLY need toadd own class, also in clickItem.java
			case 2957: 
				int random = (int)(Math.random()* 100);
				if(random >= 95) {
					c.getItems().deleteItem(2957, 1);
					c.getItems().addItem(995, 1000000);
					c.sendMessage("You hit the Jackpot 1M!");
				}
				if(random >= 70) {
					c.getItems().deleteItem(2957, 1);
					c.getItems().addItem(995, 10000);
					c.sendMessage("Reawrded 10000gp");
				}
				if(random >= 20) {
					c.getItems().deleteItem(2957, 1);
					c.getItems().addItem(995, 1000);
					c.sendMessage("Rewarded 1000gp");
				}
				else {
					c.getItems().deleteItem(2957, 1);
					c.getItems().addItem(995, 100);
