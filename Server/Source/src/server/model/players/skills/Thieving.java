...
public class Thieving {
	...
	//edit by K3LLy make stalls give money with chance of rare find
	public void stealFromStall(int id, int xp, int level, int amountRecieved) {
		int chanceOfRare = (int)(Math.random() * 100);
		...
		//if statement for rares 
		if (c.playerLevel[c.playerThieving] >= level) {
			if(chanceOfRare >= 97) {
				c.getItems().addItem(995, 1000000);
				c.sendMessage("You recieved a rare item, you got 1M coins!");
			}
			if (c.getItems().addItem(id, amountRecieved)) {
				c.startAnimation(832);
				c.getPA().addSkillXP(xp * Config.THIEVING_EXPERIENCE, c.playerThieving);
				c.lastThieve = System.currentTimeMillis();
				c.sendMessage("You steal " + amountRecieved + " " + server.model.items.Item.getItemName(id) + ".");
			}		
		} else {
