...
public class CombatAssistant{
	...
	public void attackNpc(int i) {		
		if (Server.npcHandler.npcs[i] != null) {
			if (Server.npcHandler.npcs[i].isDead || Server.npcHandler.npcs[i].MaxHP <= 0) {
				//for quest line determine monster 
				//K3LLY
				if(c.quest1Part == 1)
					questMonster(1341);
				else if(c.quest1Part == 4)
					questMonster(1158);
			...
			if(c.attackTimer <= 0) {
				...
				//implemented by K3LLY better for mage to range switch and vice versa
				if(usingBow || c.usingMagic || usingOtherRangeWeapons || (c.goodDistance(c.getX(), c.getY(), Server.npcHandler.npcs[i].getX(), Server.npcHandler.npcs[i].getY(), 2) && usingHally())) {
					c.stopMovement();
					//this resetPlayerAttack is used to make the player stand 5 distance while using bow or magic attack!
					if(c.usingMagic) {
						resetPlayerAttackExceptMagicNPC();
					}
					else if(usingBow || usingOtherRangeWeapons) {
						resetPlayerAttackExceptRangeNPC();
					} else {
						resetPlayerAttack();
					}
				}
		...
		//	}
			...
			if(c.attackTimer <= 0) {
				...
				if(usingBow || c.usingMagic || usingOtherRangeWeapons || usingHally()) {
					//same as npc fighting but just dont calculate the npc distance
					c.stopMovement();
					if(c.usingMagic) {
						resetPlayerAttackExceptMagic();
					} else {
						resetPlayerAttack();
					}
				}
				...
				if(c.duelStatus != 5) {
					try {
						...
						//need this catch block to catch the server handle error of people not in array
						//all this does is cut out the if statement because it cant find player...
						c.attackedPlayers.add(c.playerIndex);
						c.isSkulled = true;
						c.skullTimer = Config.SKULL_TIMER;
						c.headIconPk = 0;
				...
				if(usingBow && !usingOtherRangeWeapons && !c.usingMagic || usingCross) { // range hit delay
					...
					if (usingCross)
						c.usingBow = true;
					c.usingBow = true;
					//need try catch incase player array breaks
					try {
						c.followId = Server.playerHandler.players[c.playerIndex].playerId;
						c.getPA().followPlayer();
						c.lastWeaponUsed = c.playerEquipment[c.playerWeapon];
	...
	**/
	...
	//methods made by K3LLY to have way better magic/range switching and tile hodling on atatck!	
	...
	//for pking with magic and range 
	public void resetPlayerAttackExceptMagic() {
		c.usingMagic = true;
		c.npcIndex = 0;
		//c.faceUpdate(0);
		//c.playerIndex = 0;
	...
	//this is for NPC fighting and staying in place 
	public void resetPlayerAttackExceptMagicNPC() {
		c.usingMagic = true;
		//c.npcIndex = 0;
		c.faceUpdate(0);
		c.playerIndex = 0;
	...
	//for fighting npcs with range 
	public void resetPlayerAttackExceptRangeNPC() {
		c.usingMagic = false;
		c.faceUpdate(0);
		c.playerIndex = 0;
		c.getPA().resetFollow();
	...
	**/
	...
	//determin the quest monster is dead comes from attNPC if(dead function)
	//K3LLY	
	public void questMonster(int npcType) {
		switch(npcType) {
			case 1341:
				c.sendMessage("Nicely done return to Juliet");
				c.quest1Part = 2;
				c.sendMessage("You are now on part: " + c.quest1Part);
			break;
			
			case 1158:
				c.sendMessage("Nicely done return to King Arthur!");
				c.quest1Part = 5;
				c.sendMessage("You are now on part: " + c.quest1Part);
			break;
		}
